...
// -------------------------------------------------------------------------------------
{
    ...
    for (int j=j0; j<=j1; j++) {
        ...
        uint64 y = ui64min9(ui64left1(a2, a1), a2, ui64right1(a2, a3),ui64left1(b2, b1), b2, ui64right1(b2, b3),ui64left1(c2, c1), c2, ui64right1(c2, c3));
...
// -----------------------------------------------------------------------------------
{
    for (int j=j0; j<=j1; j++) {
        ...
        uint64 y = ui64min9(z, a2, x, t, b2, u, v, c2, w );
...
// -----------------------------------------------------------------------------------
{
    uint64 a1, a2, a3, b1, b2, b3, c1, c2, c3, x1, x2, y1, y2, z1, z2, res; 
    ...
    for (int j=j0+1; j<=j1+1; j++) {
        ...
        res = ui64min9(a2,x1,x2,b2,y1,y2,c2,z1,z2);
        store2(Y, i, j-1, res);
...
// -----------------------------------------------------------------------------------
{
    ...
    res1 = ui64min3(a1, b1, c1);
    res2 = ui64min3(a2, b2, c2);


    for (int j=j0+1; j<=j1+1; j++) {

        x1 = ui64left1(res2, res1);

        a3 = load2(X, i-1, j);
        b3 = load2(X, i, j);
        c3 = load2(X, i+1, j); 

        res3 = ui64min3(a3, b3, c3);

        y1 = ui64right1(res2, res3);

        res = ui64min3(res2, x1, y1);
...
// ------------------------------------------------------------------------------------
{
    ...
    for (int j = j0; j<=j1-r-1; j+=3) {
        ...
        res = ui64min9(a3,x1,x2,b3,y1,y2,c3,z1,z2);
        store2(Y, i, j+1, res);
        ...
        res = ui64min9(a4,x1,x2,b4,y1,y2,c4,z1,z2);
        store2(Y, i, j+2, res);
    }
    if (r) {
...
// ----------------------------------------------------------------------------------------
{
    ...
    for (int j=j0; j<=j1; j++) {
        ...
        res = ui64min3(x, cb1, y);
        store2(Y, i, j, res); 

        x = ui64left1(cb2, ca2);
        y = ui64right1(cb2, cc2);
        
        res = ui64min3(x, cb2, y);
        store2(Y, i+1, j, res);
...
// --------------------------------------------------------------------------------------------
{
    ...
    if (r) {
        line_erosion3_ui64matrix_swp64_elu2_red(X, i-1, j0, j1, Y);
    }
}
// ---------------------------------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------------------------------
{
    ...
    if (r) {
        line_erosion3_ui64matrix_swp64_elu2_red_factor(X, i-1, j0, j1, Y);
    }
}
// -------------------------------------------------------------------------------------------------
...
// -------------------------------------------------------------------------------------------------
{
    ...
    if (r) {
        line_erosion3_ui64matrix_swp64_ilu3_elu2_red(X, i-1, j0, j1, Y);
    }
}
// --------------------------------------------------------------------------------------------------------
